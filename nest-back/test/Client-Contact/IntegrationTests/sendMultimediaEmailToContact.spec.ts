...
describe("Integration tests of the function sendMultimediaEmailToContact in client-contact", () => {
  beforeEach(async () => {
    Jwt = new JwtService({
  ...
  test("Testing valid parameters passed into the sendMultimediaEmailToContact in client-contact", async () => {
    ...
    expect(await clientContactService.sendMultimediaEmailToContact(contacts, theUser.userID, v4())).toBe("Email sent!")
  })
})
