...
describe("Unit tests of the function sendVideoEmailToContact in client-contact", () => {
  beforeEach(() => {
    Jwt = new JwtService({
  ...
  test("Testing empty contact parameters passed into the sendPDFEmailToContact in client-contact", async () => {
    let emptyObject
    await expect(clientContactService.sendVideoEmailToContact(emptyObject, v4(), v4())).rejects.toThrow("Parameters can not be left empty")
  })
  ...
  test("Testing empty plannerID passed into the sendPDFEmailToContact in client-contact", async () => {
    ...
    await expect(clientContactService.sendVideoEmailToContact(contacts, "", v4())).rejects.toThrow("Parameters can not be left empty")
  })
  ...
  test("Testing valid parameters passed into the sendPDFEmailToContact in client-contact", async () => {
    const contacts = [
      {
        contactId: v4(),
    ...
    expect(await clientContactService.sendVideoEmailToContact(contacts, v4(), workoutID)).toBe("Email sent!")
  })
})
